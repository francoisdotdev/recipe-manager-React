...
const difficultyOrder: Record<string, number> = { Facile: 1, Moyen: 2, Difficile: 3 }
...
export function RecipeList() {
  ...
  const filteredAndSorted = useMemo(() => {
    const filtered = allRecipes.filter((recipe) => {
      ...
      return matchesSearch && matchesType && matchesDifficulty && matchesCuisine && matchesFavorites
    })
    ...
    return [...filtered].sort((a, b) => {
      switch (sortKey) {
        ...
        case 'time': return (a.prepTime + a.cookTime) - (b.prepTime + b.cookTime)
        case 'difficulty': return (difficultyOrder[a.difficulty] ?? 0) - (difficultyOrder[b.difficulty] ?? 0)
        case 'name': return a.name.localeCompare(b.name)
        default: return 0
      }
    })
  }, [allRecipes, searchTerm, typeFilter, difficultyFilter, cuisineFilter, sortKey, showFavoritesOnly, favorites])
  ...
  return (
    <div className="recipe-list">
      <div className="filters">
        ...
        <select value={cuisineFilter} onChange={(e) => setCuisineFilter(e.target.value)} className="filters-select">
          <option value="">Toutes cuisines</option>
          {CUISINES.map((c) => (
            <option key={c} value={c}>{c}</option>
          ))}
        </select>
        <select value={sortKey} onChange={(e) => setSortKey(e.target.value as SortKey)} className="filters-select filters-sort">
      ...
      <p className="recipe-list-count">{filteredAndSorted.length} recette{filteredAndSorted.length > 1 ? 's' : ''}</p>
